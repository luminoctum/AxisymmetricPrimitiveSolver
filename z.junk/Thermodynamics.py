...
def SatVaporP(sp, T):
    if isinstance(sp, list) or isinstance(sp, ndarray):
        return amap(lambda s: 10. ** (s.al + s.bl / T), sp)
    else:
...
def SatMixingR(sp, T, P):
    if not isinstance(T, ndarray):
        return SatMixingRScalar(sp, T, P)
    else:
        dims = T.shape
        n = multiply.reduce(dims)
        result = amap(lambda x, y: SatMixingRScalar(sp, x, y), T.reshape(n), P.reshape(n))
...
def IsSat(sp, mixr, temp, ptol):
    return less(SatVaporP(sp, temp), mixr / (1 + sum(mixr)) * ptol)

class CloudLayer:
    def __init__(self, sp, ptol, temp, mixr):
        self.species = sp
        self.ptol = ptol
        self.temp = temp
        self.mixr = mixr
...
# Atmosphere
class Atmosphere:
    def __init__(self, Pref = 1.E5, Tref = 134.8, grav = 10.44, mu = 2.198E-3, cp = 11456.):
        self.Pref = Pref
        self.Tref = Tref
        self.grav = grav
        self.mu = mu
    ...
    def PotentialT(self, T, p, species = []):
        # Emanuel 4.5.11
        result = T * (self.Pref / (p - sum(SatVaporP(species, T), axis = 0)))**(self.Rdry / self.cpdry)
        ...
        if isinstance(species, list) or isinstance(species, ndarray):
            ...
            else: return result * prod(exp(self.eps(species) * LatentH(species) * MixingR(SatVaporP(species, T), p) / (self.cpdry * T)))
        else: return result * exp(self.eps(species) * LatentH(species) * MixingR(SatVaporP(species, T), p) / (self.cpdry * T))

    def VirtualTScalar(self, s, T, x):
        return T * (1. + sum(x)) / (1. + sum(self.eps(s) * x))

    def VirtualT(self, s, T, X):
        if not isinstance(T, ndarray):
            result = self.VirtualTScalar(s, T, X)
            return result
        else:
            # ndarray might be zero shape and looks exactly like a float!
            dims = T.shape
            n = size(T)
            result = amap(lambda x, y: self.VirtualT(s, x, y), T.reshape(n), X.reshape(n, 2))
            return result.reshape(dims)

    def SatVirtualT(self, s, T, p):
        return self.VirtualT(s, T, SatMixingR(s, T, p))
    ...
    def EqCondensation(self, sp, T, P, X, pmin, pmax, num = 200):
        sp = array(sp)
        XS = array([s.mmr for s in sp])
        mixr = amap(min, X, XS)
        sat = IsSat(sp, mixr, T, P)
        sat0 = sat.copy()
        ...
        while (i >= 0 and sum(logical_not(sat)) > 0):
            ...
            newsat = IsSat(sp, mixr, temp, plev[i])
            ...
            xcloud = array([mixr[ii] if not sat[ii] else SatVaporP(sp[ii], temp) / pdry for ii in range(len(sp))])
            cloud = [CloudLayer(sp[j], ptol, temp, xcloud)] + cloud
            ...
            pdry = (_sample_p - sum(SatVaporP(sp[sat], _sample_t), axis = 0)) / (1. + sum(mixr[logical_not(sat)]))
            _sample_x = zeros((len(sp), len(pdry)))
            for k in range(len(sp)):
                if sat[k]: _sample_x[k, :] = SatVaporP(sp[k], _sample_t) / pdry
        ...
        while (i < num and sum(sat) > 0):
            ...
            #print "Find Cloud Layer: T = %8.2f, P = %8.2f, Pdry = %8.2f, Species = " % (temp, ptol, pdry), sp[j]
            xcloud = array([mixr[ii] if not sat[ii] else SatVaporP(sp[ii], temp) / pdry for ii in range(len(sp))])
            cloud = cloud + [CloudLayer(sp[j], ptol, temp, xcloud)]
    ...
    def _eq1_(self, var, sp, sat, pt, mixr, j):
        ...
        return (self.PotentialT(tc, pc, sp[sat]) - pt,
                SatVaporP(sp[j], tc) - mixr[j] * pd,
                pd * (1. + sum(mixr[logical_not(sat)])) + sum(SatVaporP(sp[sat], tc)) - pc
                )
    def _eq2_(self, mixr, sp, vt, ptol):
        tc = vt * (1. + sum(self.eps(sp) * mixr)) / (1. + sum(mixr))
        return SatVaporP(sp, tc) - mixr * ptol / (1. + sum(mixr))
    def __repr__(self):
